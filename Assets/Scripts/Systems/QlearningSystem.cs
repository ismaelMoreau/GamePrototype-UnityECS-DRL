...
//         foreach(var enemy in SystemAPI.Query<RefRW<EnemyActionComponent>>()){
...
//                 int nextActionIndexPosition = GetNeighborIndices(positionInitial,chosenAction,gridconfig.width,gridconfig.height);
...
//         int up = row > 0 ? index - width : -1; // Move up if not in the first row
//         int down = row < height - 1 ? index + width : -1; // Move down if not in the last row
//         int right = col < width - 1 ? index + 1 : -1; // Move right if not in the last column
//         int left = col > 0 ? index - 1 : -1; // Move left if not in the first column
...
//     private (int action, float value) SelectAction(int index, float epsilon, 
//         float4 possibleMovAction, int oldAction, int width, int height) {
...
//     private (int index, float value) GetMaxValueActionFromValidActions(float4 possibleMovAction, NativeList<int> validActions) {
